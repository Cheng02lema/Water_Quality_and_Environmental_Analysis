% Script: plot_Nitrogen_Normalized.m
...
% 归一化处理（Min-Max 归一化）
nitrogen_norm = (nitrogen - min(nitrogen)) / (max(nitrogen) - min(nitrogen));
...
% 设置图表标题
title('无机氮 (氮) 的时间序列折线图 (归一化)', 'FontSize', 14, 'FontWeight', 'bold');
...
% 保存图表为PNG文件（可选）
saveas(gcf, 'Nitrogen_Normalized.png');
